...
var (
	userColumns               = []string{"id", "created_at", "updated_at", "email", "password"}
	userColumnsWithoutDefault = []string{"created_at", "updated_at", "email", "password"}
...
func (userL) LoadAccounts(e boil.Executor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(qm.From(`accounts`), qm.WhereIn(`user_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
